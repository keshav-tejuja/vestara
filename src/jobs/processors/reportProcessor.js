  ...
  try {
    ...
    for (const { portfolio_id, user_id, name, email } of portfolios) {
      try {
        ...
        // Get real weekly performance
        const weeklyPerf = await PriceHistory.getWeeklyPerformance(portfolio_id);
        ...
        const weeklySummary = await generateWeeklyReport({
          ...
          weeklyChange: weeklyPerf?.weekly_change_percent?.toFixed(2) || 0
        });
